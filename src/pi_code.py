...
def _relay_off():
    ...
    # Always acquire lock before any relay state change
    with relay_lock:
...
while running:
    ...
    # ── Classification result ────────────────────────────────────
        ...
        if result == "Good":
            ...
            # Acquire lock for entire operation: cancel timer + set relay state
            with relay_lock:
        ...
        # DEFECT bottle
            ...
            # Acquire lock for entire operation: cancel old timer + set relay + create new timer
            with relay_lock:
                if classify_off_timer is not None:
                    classify_off_timer.cancel()
                ...
                # Create and start timer while holding lock to ensure atomic state
